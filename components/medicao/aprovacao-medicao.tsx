...
export function AprovacaoMedicao({ token }: { token: string }) {
  ...
  async function confirmar() {
    ...
    setEnviando(true);
    try {
      const res = await fetch(`/api/publico/medicoes/${token}/aprovar`, {
        method: "POST",
  ...
  if (!aberto) {
    return (
      <div className="bg-gradient-to-br from-primary-500 to-success-500 rounded-2xl p-6 text-center shadow-elevated">
        <h3 className="text-white text-xl font-bold mb-2">Aprovar esta medição</h3>
        <p className="text-primary-50 text-sm mb-4">Confirme sua aprovação digitalmente com sua assinatura.</p>
        <button
          type="button"
        ...
        >
          <PenLine className="w-5 h-5" /> Aprovar medição
        </button>
      </div>
    );
  }
    ...
    <div className="bg-white border-2 border-success-500 rounded-2xl p-6 space-y-4 shadow-elevated">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-ink flex items-center gap-2">
          <CheckCircle2 className="w-5 h-5 text-success-600" /> Confirmar aprovação
        </h3>
      ...
      <div className="space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <button type="button" onClick={() => sigRef.current?.clear()} className="text-xs text-ink-muted hover:text-primary-600 inline-flex items-center gap-1">
